...
class ED:
    ...
    def add_level(self, energy, bottom_text='', position=None, color='k', alpha=1,
        ...
        if bottom_text == 'Energy':
            if self.round_energies_at_digit == "keep all digits":
                bottom_text = energy
            else:
                bottom_text = round(energy,self.round_energies_at_digit)
    ...
    def plot(self, show_IDs=False, ylabel="Energy / $kcal$ $mol^{-1}$", level_width=3, ax: plt.Axes = None):
        '''
        Method of ED class
        ...
        for idx, link in enumerate(self.links):
            if len(self.labels)>label_id and self.positions[idx]==0:
